...
Template for scalable Telegram bots on **aiogram 3.x** (stable, pydantic v2). Python ≥3.14 (`requires-python`), uv.
...
- **Models**: `models/base.py` — project-wide pydantic v2 `BaseModel` base (empty hook for shared `model_config`); also holds `orjson_dumps` used by the JSON log renderer.
